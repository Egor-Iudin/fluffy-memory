...
#include "/afs/cern.ch/user/d/doudko/work/public/bnn/TrainEventList.hpp"
 ...
 TH1D &trainHist, TH1D &examHist)
{
    ...
    {
        const bool res = trainList.ReadEventList(*fName);
        if (!res) 
        {
            cout << "ERROR: fail to read event list" << endl;
            exit(1);
        }
        ...
        // Calculated the weighted factor for histogram normalization (train and exam hists are different, but we want to compare them)
        double n_total_weighted = 0.;
        double n_train_weighted = 0.;
        for (long ev = 0; ev < nEntries; ++ev)
        {
            tree->LoadTree(ev);
            Double_t const weight = weightFormula.EvalInstance();
            n_total_weighted += weight;
            
            if (!(trainList.CheckEventExam(ev)))
                n_train_weighted += weight;
        }
        
        //If train or exam hist is zero go to the next file
        if ((n_train_weighted == 0) or (n_train_weighted == n_total_weighted))
            continue;
        
        double const examWeightFactor = 1. - n_train_weighted / n_total_weighted;        
...
void PlotDiscrimination::Draw(std::string const &title, std::string const &plotFileName)
{
    ...
    //Calculate XDif and provide some decoration
    double hist_max = std::max(bkgTrainHist.GetMaximum(),sgnTrainHist.GetMaximum());
    sgnTrainHist.SetMaximum(1.1 * hist_max);
    sgnTrainHist.SetMinimum(0.);
    
    double x_area = 0.;
    for (int bin = 1; bin <= nBins; ++bin)
        x_area += std::min(bkgExamHist.GetBinContent(bin),sgnExamHist.GetBinContent(bin));
    double x_dif = 1. - x_area;

    TPaveText xdifResult(0.6, 0.83, 0.9, 0.87, "NDC");

    xdifResult.SetFillColor(0);
    xdifResult.SetTextAlign(31);
    xdifResult.SetBorderSize(0);

    std::ostringstream ost;
    ost.precision(3);
    ost << "XDif value: " << x_dif;
    xdifResult.AddText(ost.str().c_str());
    
    float const margin = 0.1;
    TPaveText titlePave(margin, 1. - margin, 1. - margin, 1., "NDC");
        
        
            titlePave.SetTextFont(23);
            titlePave.SetTextSize(40);
            titlePave.SetTextAlign(11); //instead of 22
            titlePave.SetFillStyle(0);  // transparent
            titlePave.SetBorderSize(0);
            
            titlePave.AddText("CMS preliminary, #sqrt{s} = 13 TeV, #it{L} = 35.9 fb^{-1}");
            
        
    
    //create the description
    TPaveText descr(0.15, 0.84, 0.35, 0.89, "NDC");

    descr.SetFillColor(0);
    descr.SetTextAlign(31);
    descr.SetBorderSize(0);

    descr.AddText("CMS preliminary");
    ...
    // The legend
    TLegend legend(0.4, 0.6, 0.6, 0.88);
    //TLegend legend(0.12, 0.55, 0.32, 0.83); //for LVRV BNNs
